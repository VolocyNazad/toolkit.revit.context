...
Source for `VolocyNazad.Revit.Context`: an abstraction over the Revit
API/UI application context (which thread, which document is active, etc.)
for other libraries to depend on rather than talking to the raw Revit API
directly. Consumed by `toolkit.revit.mediatR` and
`toolkit.revit.transaction-cache` in this same family, and by
`Impact.Toolkit.Revit.Context.DependencyInjection` /
`impact.revitaddinapi` downstream.
...
- Tests: **xunit.v3** (matches `toolkit.revit.async` in this same repo
  family) + coverlet.collector + Microsoft.Extensions.DependencyInjection
